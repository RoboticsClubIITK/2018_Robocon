...
while cv.waitKey(1) & 0xff!=ord('q'):
    ...
    ret,th1 = cv.threshold(blur,35,255,cv.THRESH_BINARY+cv.THRESH_OTSU)#using threshold remave noise
    ...
    im=[im1,im2,im3,im4]
    for i in range(0,4):
        _,contours, hierarchy = cv.findContours(im[i],cv.RETR_TREE,cv.CHAIN_APPROX_SIMPLE) #find the contours
        if len(contours)>0:
            ...
            if area>=500:
                ...
                cv.drawContours(frame[int((3-i)*h/4):int((4-i)*h/4),:],cnt,-1,(0,255,0),3)
